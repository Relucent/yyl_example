 ...
 * 移动的SCV (Canvas用法演示)
 */
public class CanvasBoard extends Canvas implements Runnable {
	...
	public CanvasBoard() {
		Image image = Toolkit.getDefaultToolkit().createImage(this.getClass().getResource("0_scv_red.gif"));
		for (int i = 0; i < 5; i++)
			images[i] = Toolkit.getDefaultToolkit()
					.createImage(new FilteredImageSource(image.getSource(), new CropImageFilter(0, 47 * i, 45, 45)));
	...
	public void paint(Graphics gc) {
		...
		if (state < 0) {
			gc.drawImage(images[-state], starPoint.x - 25, starPoint.y - 25, starPoint.x + 20, starPoint.y + 20, 50, 0,
					0, 50, this);
		} else {
			gc.drawImage(images[state], starPoint.x - 25, starPoint.y - 25, starPoint.x + 20, starPoint.y + 20, 0, 0,
					50, 50, this);
		}
	}
