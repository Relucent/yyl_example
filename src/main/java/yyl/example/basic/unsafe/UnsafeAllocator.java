...
/**
 * 不经过构造函数生成对象实例(不安全的分配器)<br>
 * JDK5~JDK7 测试可以使用
 */
//算法参照: com.google.gson.internal
	...
	public static UnsafeAllocator create() {
		try {
			...
			try {
				...
				try {
					Method getConstructorId = ObjectStreamClass.class.getDeclaredMethod("getConstructorId",
							new Class[] { Class.class });

					getConstructorId.setAccessible(true);
					final int constructorId = ((Integer) getConstructorId.invoke(null, new Object[] { Object.class }))
							.intValue();
					final Method newInstance = ObjectStreamClass.class.getDeclaredMethod("newInstance",
							new Class[] { Class.class, Integer.TYPE });

					newInstance.setAccessible(true);
					return new UnsafeAllocator() {
						public <T> T newInstance(Class<T> c) throws Exception {
							return (T) newInstance.invoke(null, new Object[] { c, Integer.valueOf(constructorId) });
						}
					};
